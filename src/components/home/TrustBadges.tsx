    ...
    <ul className={`flex flex-wrap gap-3 ${className}`}>
      {trustBadges.map((badge, index) => {
        ...
        return (
          <li
            key={badge.title}
            className="flex items-center gap-3 rounded-[14px] border border-border bg-white/85 px-4 py-3 shadow-card backdrop-blur-sm"
          >
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-soft text-accent-deep">
              <Icon className="h-[18px] w-[18px]" aria-hidden="true" />
            </span>
            <span className="max-w-[11.5rem] leading-snug">
